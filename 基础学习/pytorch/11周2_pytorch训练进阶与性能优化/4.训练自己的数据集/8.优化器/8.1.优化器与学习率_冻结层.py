# 基于以下文件做的修改，可以对比着看
# /Users/yafei/practice/python-practice/基础学习/pytorch/11周2_pytorch训练进阶与性能优化/4.训练自己的数据集/6.使用更强大的模型训练.py
import torch
...
def writedata(
    ...
    for name, param in model.named_parameters():
        tensorboard_writer.add_histogram(
            name, param.clone().cpu().data.numpy(), global_step=epoch
        )
...
if __name__ == "__main__":
    ...
    for name, value in pretrain_model.named_parameters():
        # 只冻结全连接层：如果不是fc相关的，就冻结梯度信息，结果就是这些层就不会更新了
        if (name != "fc.weight") and (name != "fc.bias"):
            value.requires_grad = False
    #
    # filter 函数将模型中属性 requires_grad = True 的参数选出来
    params_selected = filter(
        lambda p: p.requires_grad, pretrain_model.parameters()
    )  # 要更新的参数在 params_selected 当中
